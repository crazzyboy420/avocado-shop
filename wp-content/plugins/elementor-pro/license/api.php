...
class API {
	...
	public static function is_license_active() {
		...
		return true; //self::STATUS_VALID === $license_data['license'];
	}

	public static function is_license_expired() {
		$license_data = self::get_license_data();

		return false; //self::STATUS_EXPIRED === $license_data['license'];
	}

	public static function is_licence_pro_trial() {
		return false; //self::is_licence_has_feature( self::FEATURE_PRO_TRIAL );
	}

	public static function is_licence_has_feature( $feature_name ) {
		$license_data = self::get_license_data();

		return true; //in_array( $feature_name, $license_data['features'], true );
	}
	...
	public static function is_license_about_to_expire() {
		...
		if ( 'lifetime' === $license_data ) {
			return false;
		}

		return false; //time() > strtotime( '-28 days', strtotime( $license_data['expires'] ) );
	}
